...
output: 
  html_document:
    theme: cosmo #default #cosmo #lumen # cosmo #spacelab #cerulean #flatly
    highlight: tango # textmate #tango  #null
    toc: true
    toc_depth: 2
...
\end{equation}
To distinguish these two models, we shall call the former as a latent GP model, and the latter as a response GP model. These two models are referred as the latent variable GP and the marginal likelihood GP in Stan reference manual (@tstan2017).
...
One can readily calculate the determinant of $C^*$ by the product of the diagonal elements in $D^*$. The likelihood of $w(S)$ based on precision matrix $C^{*-1}$ serves as a good approximation to the likelihood of $w(s)$ in \\eqref{eq: latent model}, while the storage and computational burden of the former is linear in $N$.  
