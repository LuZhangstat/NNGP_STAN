...
\end{equation}
To distinguish these two models, we shall call the former as a random-effects model, and the latter as a response model. These two models are referred as the latent variable GP and the marginal likelihood GP in Stan reference manual (@tstan2017).
...
We assign Gaussian priors for $\sigma$ and $\tau$, an inverse gamma prior for $\phi$ and a Gaussian prior for $\beta = \{\beta_0, \beta_1\}$. The following is the parameter and model block for a random-effects NNGP model.
```{stan output.var="test", eval = F}
  parameters{
      vector[P + 1] beta;
      real<lower = 0> sigma;
...
```{r}
writeLines(readLines('nngp_random.stan'))
```
...
stan_trace(samples)
```
We also run the model with marginal GP likelihood as a benchmark for comparison. For the sake of simplicity, we suppress the full code and give the summary table below. The Stan code "GP_marginal.stan" and R code "nngp.R" are posted on the Github. The posterior estimates of the parameters from two models are similar, while the running time required for response NNGP model is much less than the basic GP. The efficiency of NNGP will be more obvious when the dataset gets larger.
